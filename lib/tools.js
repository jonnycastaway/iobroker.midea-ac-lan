...
class NetHomeCloud {
    ...
    async request(path, data, timeoutMs = 10000, includeSession = true) {
        return new Promise((resolve, reject) => {
            ...
            const queryStr = sortedKeys.map(k => k + '=' + String(body[k])).join('&');
            
            const msg = path + queryStr + NETHOME_CONFIG.APP_KEY;
            ...
            const postData = sortedKeys.map(k => k + '=' + String(body[k])).join('&');
            this.log('debug', 'Post data: ' + postData);

            const req = https.request(options, (res) => {
                let responseBody = '';
                res.on('data', chunk => responseBody += chunk);
                res.on('end', () => {
                    this.log('debug', 'NetHome raw response: ' + responseBody);
                    try {
                        const json = JSON.parse(responseBody);
