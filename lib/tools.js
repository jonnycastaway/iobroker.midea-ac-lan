...
class MideaProtocol {
    ...
    calculateTcpKey(responseData) {
        // responseData is from byte 8 onwards = 64 bytes
        // First 32 = encrypted, last 32 = signature
        // Decrypt with key, verify, XOR
        ...
        // Decrypt encrypted part with key (AES-CBC with zero IV)
        ...
        try {
            ...
            console.log('[MideaProtocol] DECRYPT FAILED:', e.message);
            return null;
        }
        
        // Verify signature (SHA256 of plain should equal signature)
        const hash = crypto.createHash('sha256').update(plain).digest();
        let match = true;
        for(let i = 0; i < 32; i++) {
            if (hash[i] !== signaturePart[i]) {
                match = false;
                break;
            }
        }
        if (!match) {
            console.log('[MideaProtocol] Signature mismatch!');
            return null;
        }
        
        // XOR plain with key to get tcpKey (32 bytes)
        const tcpKey = Buffer.alloc(32);
        for (let i = 0; i < 32; i++) {
            tcpKey[i] = plain[i] ^ this.key[i];
        }
        
        // Store as 64 bytes (copy twice for compatibility)
        this.tcpKey = Buffer.concat([tcpKey, tcpKey]);
        console.log('[MideaProtocol] tcpKey calculated:', this.tcpKey.toString('hex').substring(0, 32) + '...');
        return this.tcpKey;
    }
...
sendApplianceQuery() {
        ...
        // Debug: show what's being encrypted
        if (this.protocol.tcpKey) {
            console.log('[DEBUG] Sending query:', packet.toString('hex'));
            console.log('[DEBUG] tcpKey:', this.protocol.tcpKey.toString('hex').substring(0, 32));
        }
        
        const encrypted = this.protocol.encode8370(packet, MSGTYPE_ENCRYPTED_REQUEST);
        console.log('[DEBUG] Encrypted:', encrypted.toString('hex').substring(0, 60) + '...');
