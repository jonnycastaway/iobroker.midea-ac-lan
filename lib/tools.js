...
class MideaAC {
    ...
    sendHandshake() {
        ...
        this.log.info(`[${this.deviceId}] Token hex len: ${this.config.token.length}, buffer len: ${payloadLen}`);
        
        // Explicit size calculation
        const size = payloadLen;
        this.log.info(`[${this.deviceId}] Size to write: ${size} (0x${size.toString(16)})`);
        
        const frame = Buffer.alloc(8 + size);
        frame[0] = 0x83;
        frame[1] = 0x70;
        // Write size as 2 bytes big-endian
        frame[2] = (size >> 8) & 0xFF;  // high byte
        frame[3] = size & 0xFF;           // low byte
        this.log.info(`[${this.deviceId}] Written size: byte2=0x${frame[2].toString(16)}, byte3=0x${frame[3].toString(16)}`);
        
        frame[4] = MSGTYPE_HANDSHAKE_REQUEST;
        frame[5] = 0;
        frame[6] = 0;
        frame[7] = 0;
        payload.copy(frame, 8);
        
        this.log.info(`[${this.deviceId}] Full handshake: ${frame.toString('hex')}`);
        this.socket.write(frame);
    }
    
    processMessage(packet) {
        this.log.info(`[${this.deviceId}] Received packet: ${packet.length} bytes, hex: ${packet.toString('hex').slice(0,40)}...`);
