...
class NetHomeCloud {
    ...
    async request(path, data, timeoutMs = 10000, includeSession = true) {
        return new Promise((resolve, reject) => {
            ...
            const queryDecoded = queryEncoded.replace(/\+/g, ' ');
            
            const msg = path + queryDecoded + NETHOME_CONFIG.APP_KEY;
            ...
            const postDataSorted = Object.keys(body).sort().map(k => k + '=' + String(body[k])).join('&');
            this.log('info', 'Post data: ' + postDataSorted);
