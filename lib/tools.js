...
const MSGTYPE_ENCRYPTED_RESPONSE = 0x07;
...
class MideaAC {
    ...
    async connect() {
        return new Promise((resolve, reject) => {
            ...
            this.socket.on('connect', () => {
                this.log.info(`[${this.deviceId}] TCP connected, sending handshake`);
                this.sendHandshake();
                
                setTimeout(() => {
                    if (!this.connected) {
                        this.log.warn(`[${this.deviceId}] No response after 2 seconds, checking socket state`);
                        this.log.info(`[${this.deviceId}] Socket writable: ${this.socket.writable}`);
                        this.log.info(`[${this.deviceId}] Socket destroyed: ${this.socket.destroyed}`);
                    }
                }, 2000);
            });
            
            this.socket.on('data', (data) => {
                this.log.info(`[${this.deviceId}] DATA EVENT - Received ${data.length} bytes: ${data.toString('hex').substring(0, 150)}`);
                this.handleData(data);
            });
            
            this.socket.on('timeout', () => {
                this.log.warn(`[${this.deviceId}] Connection timeout`);
                this.available = false;
            });
            ...
            this.socket.on('close', () => {
                this.log.info(`[${this.deviceId}] Connection closed`);
                this.connected = false;
                this.available = false;
            });
    ...
    sendHandshake() {
        ...
        this.log.info(`[${this.deviceId}] Sending handshake (${request.length} bytes): ${request.toString('hex')}`);
        
        const written = this.socket.write(request, (err) => {
            if (err) {
                this.log.error(`[${this.deviceId}] Write error: ${err.message}`);
            } else {
                this.log.info(`[${this.deviceId}] Handshake sent successfully`);
            }
        });
        
        this.log.info(`[${this.deviceId}] Socket write returned: ${written}`);
    }

    handleData(data) {
        const hex = data.toString('hex');
        this.log.info(`[${this.deviceId}] Received ${data.length} bytes: ${hex.substring(0, 100)}`);
        
        if (!this.connected) {
            this.log.info(`[${this.deviceId}] Processing handshake response...`);
            
            if (data.length >= 72) {
                const responseData = data.slice(8, 72);
        ...
        const decrypted = this.protocol.decryptResponse(data);
        if (decrypted) {
            this.parseResponse(decrypted);
        }
    }

    async refreshStatus() {
        if (!this.connected || !this.socket) {
            this.log.warn(`[${this.deviceId}] Not connected, skipping refresh`);
            return;
        }
        
        const query = this.buildQueryAppliance();
        const encrypted = this.protocol.encode8370(query, MSGTYPE_ENCRYPTED_REQUEST);
        
        this.log.info(`[${this.deviceId}] Sending query (${query.length} bytes payload): ${query.toString('hex')}`);
        this.log.info(`[${this.deviceId}] Encrypted: ${encrypted.toString('hex')}`);
        
        this.socket.write(encrypted, (err) => {
            if (err) {
                this.log.error(`[${this.deviceId}] Write error: ${err.message}`);
            } else {
                this.log.info(`[${this.deviceId}] Query sent, waiting for response...`);
            }
        });
    }
    ...
    buildQueryAppliance() {
        ...
        const deviceIdStr = this.deviceId.toString();
        const deviceIdNum = parseInt(deviceIdStr, 10);
        
        header.writeUIntLE(deviceIdNum, 3, 6);
        ...
        const checksum = packet.slice(1).reduce((sum, b) => sum + b, 0) & 0xFF;
    ...
    parseResponse(data) {
        if (typeof data === 'string') {
            if (data.includes('ERROR')) {
                this.log.warn(`[${this.deviceId}] Device returned ERROR`);
                this.available = false;
            }
            return;
        }
        ...
        const msgType = data[9];
        
        if (msgType === 0xA0) {
            this.log.info(`[${this.deviceId}] Appliance response received`);
            this.available = true;
            this.parseApplianceResponse(data);
        } else if (msgType === 0x03) {
            this.parseStatusResponse(data);
        }
    }

    parseApplianceResponse(data) {
        if (data.length < 39) return;
        
        this.available = true;
        
        this.log.info(`[${this.deviceId}] Device available, fetching status...`);
        
        this.refreshStatusDetailed();
    }

    async refreshStatusDetailed() {
        if (!this.connected || !this.socket) return;
        
        const query = this.buildStatusQuery();
        const encrypted = this.protocol.encode8370(query, MSGTYPE_ENCRYPTED_REQUEST);
    ...
    buildStatusQuery() {
        ...
        header[1] = 0x28;
        header[2] = DEVICE_TYPE_AC;
        header.writeUIntLE(parseInt(this.deviceId, 10), 3, 6);
        header[7] = 0x00;
        header[8] = 0x00;
        header[9] = 0x03;
    ...
    apply() {
        if (!this.connected || !this.socket) {
            this.log.warn(`[${this.deviceId}] Not connected, cannot apply`);
            return;
        }
        
        this.log.info(`[${this.deviceId}] Applying settings...`);
...
class MideaAdapter {
    ...
    startDevice() {
        ...
        if (!deviceId || !ipAddress || !token || !key) {
            this.adapter.log.error('Missing required configuration: deviceId, ipAddress, token, key');
            return;
        }
    ...
    async connectDevice() {
        try {
            ...
            this.adapter.log.info('Starting status refresh every ' + (interval / 1000) + ' seconds');
            
            this.refreshInterval = setInterval(() => {
                if (this.device && this.device.connected) {
                    this.device.refreshStatus();
                }
            }, interval);
    ...
    createDeviceStates(deviceId) {
        const states = [
            ...
            { id: 'targetTemperature', type: 'number', role: 'value.temperature', write: true, min: 16, max: 30 },
