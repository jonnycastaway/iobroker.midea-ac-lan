...
class MideaProtocol {
    ...
    encode8370(data, msgType) {
        ...
        if (msgType === MSGTYPE_ENCRYPTED_REQUEST) {
            ...
            if ((dataLen + 2) % 16 !== 0) {
                padding = 16 - ((dataLen + 2) & 0xF);
                size += padding + 32;
            }
            
            let dataWithPadding = Buffer.concat([payload]);
            if (padding > 0) {
                const pad = Buffer.alloc(padding);
                for(let i = 0; i < padding; i++) pad[i] = padding;
                dataWithPadding = Buffer.concat([payload, pad]);
            }
            
            const reqCount = Buffer.alloc(2);
            reqCount.writeUInt16BE(this._requestCount++, 0);
            let finalData = Buffer.concat([reqCount, dataWithPadding]);
        ...
        frame[3] = 0x20; frame[4] = msgType;
        payload.copy(frame, 5);
        let checksum = 0;
        for(let i = 0; i < payload.length + 5; i++) checksum ^= frame[i];
...
class MideaAC {
    ...
    sendHandshake() {
        ...
        this.log.info(`[${this.deviceId}] Handshake request: ${req.toString('hex').slice(0,40)}...`);
        this.socket.write(req);
    }
    
    processMessage(packet) {
        this.log.info(`[${this.deviceId}] processMessage: ${packet.length} bytes`);
