...
class MideaProtocol {
    ...
    encode8370(data, msgType) {
        ...
        // Check if encrypted type
        if (msgType === MSGTYPE_ENCRYPTED_REQUEST || msgType === MSGTYPE_ENCRYPTED_RESPONSE) {
            if ((dataLen + 2) % 16 !== 0) {
...
class MideaAC {
    ...
    sendHandshake() {
        ...
        this.log.info(`[${this.deviceId}] Handshake: ${req.toString('hex').slice(0,40)}...`);
