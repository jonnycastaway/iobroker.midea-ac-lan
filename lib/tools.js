...
class MideaAdapter {
    ...
    async start(adapter) {
        ...
        console.log('*** MIDEA START ***');
        adapter.log.info('Midea adapter starting...');
    ...
    startRefreshLoop() {
        ...
        this.adapter.log.info('=== START_REFRESH_LOOP interval=' + interval + 'ms devices=' + this.devices.size);
        
        for (const [deviceId, device] of this.devices) {
            this.adapter.log.info('=== DEVICE_MAP id=' + deviceId + ' ip=' + device.ipAddress + ' token=' + (device.token ? 'SET' : 'MISSING') + ' key=' + (device.key ? 'SET' : 'MISSING'));
        }
        ...
        this.refreshTimer = setInterval(async () => {
            ...
            for (const [deviceId, device] of this.devices) {
                this.adapter.log.info('=== QUERY DEVICE: ' + deviceId + ' at ' + device.ipAddress + ' protocol:' + device.protocol + ' token:' + (device.token ? 'SET' : 'none') + ' key:' + (device.key ? 'SET' : 'none'));
    ...
    async queryDevice(device) {
        this.adapter.log.info('=== QUERY_DEV protocol=' + device.protocol + ' hasToken=' + !!device.token + ' hasKey=' + !!device.key);
        
        // Try msmart-style query first (non-encrypted V3 protocol)
        if (device.protocol === 3 && device.token && device.key) {
            this.adapter.log.info('=== QUERY_DEV calling Msmart');
            try {
    ...
    async queryDeviceMsmart(device) {
        ...
        this.adapter.log.info('=== MSMART_START id=' + device.id + ' ip=' + device.ipAddress + ' token=' + !!device.token + ' key=' + !!device.key);
            ...
            socket.connect(device.port, device.ipAddress, () => {
                ...
                socket.once('data', (d) => {
                    ...
                    if (d.length >= 64) {
                        ...
                        setTimeout(() => {
                            // Use msmart payload with 0x8370 wrapper
                            const query = this.buildMsmartQueryV3();
                            this.adapter.log.info('=== Msmart: Sending query V3: ' + query.toString('hex'));
                            socket.write(query);
    ...
    buildMsmartQuery() {
        ...
        this.adapter.log.info('buildMsmartQuery: length=' + length + ' payloadLen=' + payloadWithCrc.length);
        ...
        this.adapter.log.info('buildMsmartQuery: ' + result.toString('hex') + ' len=' + result.length);
        return result;
    }

    // Try V3 protocol with 0x8370 wrapper using msmart payload
    buildMsmartQueryV3() {
        const payload = Buffer.from([
            0x41, 0x81, 0x00, 0xFF, 0x03, 0xFF, 0x00, 0x02,
            0x00, 0x00, 0x00, 0x00,
        ...
        this.adapter.log.info('buildMsmartQueryV3: payload=' + payload.toString('hex'));
        
        return this.encode8370(payload, 0x03);
    }
