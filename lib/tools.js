...
class MideaProtocol {
    ...
    encode8370(data, msgType) {
        ...
        if (msgType === MSGTYPE_ENCRYPTED_REQUEST) {
            ...
            const dataWithCount = Buffer.concat([requestCount, payload]);
            
            const paddedData = this.pkcs7Pad(dataWithCount, 16);
            
            const iv = Buffer.alloc(16, 0);
            const encrypted = this.aesCbcEncrypt(paddedData, this.tcpKey, iv);
            ...
            frame = Buffer.concat([header, encrypted, signature]);
            
        } else {
            const length = payload.length;
            
            frame = Buffer.alloc(length + 9);
            frame[0] = 0x83;
            frame[1] = 0x70;
    ...
    decryptResponse(response) {
        ...
        if (msgType === 0x03) {
            ...
            const encrypted = response.slice(8, 8 + length - 2);
            const signature = response.slice(8 + length - 2);
            
            if (this.tcpKey) {
                const iv = Buffer.alloc(16, 0);
                try {
                    const decrypted = this.aesCbcDecrypt(encrypted, this.tcpKey, iv);
                    return decrypted;
                } catch (e) {
                    return null;
                }
...
class MideaAC {
    ...
    sendApplianceQuery() {
        if (!this.socket || !this.connected) {
            this.log.warn(`[${this.deviceId}] Not connected, cannot send query`);
            return;
        }
