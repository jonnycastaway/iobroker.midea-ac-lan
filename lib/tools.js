...
class NetHomeCloud {
    ...
    async request(path, data, timeoutMs = 10000, includeSession = true) {
        return new Promise((resolve, reject) => {
            ...
            // Python's urlencode + unquote_plus gives us %XX kept but + converted to space
            // But we actually need: raw key=value&key2=value2 for the sign
            // The sign in Python uses unquote_plus which converts + to space and keeps %XX
            // So we need to manually decode %XX -> raw char but keep + as +
            const queryForSign = queryEncoded.replace(/%([0-9A-Fa-f]{2})/g, (match, hex) => String.fromCharCode(parseInt(hex, 16))).replace(/\+/g, ' ');
    ...
    async getToken(udpid) {
        ...
        const data = await this.request(NETHOME_ENDPOINTS.getToken, {
            udpid: udpid
        });
    ...
    async fetchTokensForDevices() {
        ...
        try {
            ...
            for (const cloudDev of cloudDevices) {
                const ipMatch = Array.from(this.devices.values()).find(d => 
                    d.ipAddress === cloudDev.ip || d.ipAddress === cloudDev.wifiIp
                );
                
                if (ipMatch) {
                    const tokenData = await this.cloud.getToken(cloudDev.udpId);
                    if (tokenData) {
                        ipMatch.token = tokenData.token;
                        ipMatch.key = tokenData.key;
                        ipMatch.udpId = tokenData.udpId;
                        this.adapter.log.info('Got token for device at ' + ipMatch.ipAddress);
                    }
                }
            }
        } catch (e) {
