...
class MideaAdapter {
    constructor() {
        ...
        this.tcpKey = null;
        this.requestCount = 0;
        this.queryPhase = 'set';  // 'set' or 'poll'
    }
                ...
                try {
                    ...
                    socket.once('data', (d) => {
                        ...
                        if (d.length >= 64) {
                            ...
                            // Reset timeout and add 3 second delay like Python
                            socket.setTimeout(60000);
                            
                            // 3 second delay like Python - KRITISCH!
                            setTimeout(() => {
                                // First: SET command (like T_poll.py)
                                const setQuery = this.buildV3Query(device, 'set');
                                this.adapter.log.info('=== Sending SET first');
                                socket.write(setQuery);
                                
                                // Wait 2 seconds then send POLL
                                setTimeout(() => {
                                    const pollQuery = this.buildV3Query(device, 'poll');
                                    this.adapter.log.info('=== Sending POLL after SET');
                                    socket.write(pollQuery);
                                    
                                    socket.setTimeout(30000);
                                    
                                    socket.once('data', (data) => {
                                        this.adapter.log.info('=== Got response: ' + data.toString('hex').substring(0, 40));
                                        socket.destroy();
    ...
    initTcpKey(response, key) {
        try {
            ...
            this.adapter.log.info('=== DEBUG: key=' + key.toString('hex').substring(0, 32));
            
            // Use AES-256 with manual padding handling
    ...
    encode8370(data, msgType) {
        ...
        // Two bytes: 0x20 + (padding << 4) | msgType
        ...
        if (msgType === 0x00) {
            ...
            if ((msgType === 0x03 || msgType === 0x06) && this.tcpKey) {
                ...
                // First add counter (like Python line 323)
                payload = Buffer.concat([counter, payload]);
                
                // Then calculate SHA256 on header + counter + payload (like Python line 328)
                const sign = crypto.createHash('sha256').update(Buffer.concat([header, sizeBuf, flagBuf, payload])).digest();
                
                // Then AES-256 encrypt the payload (NOT including counter, but Python encrypts everything)
                const cipher = crypto.createCipheriv('aes-256-cbc', this.tcpKey, Buffer.alloc(16, 0));
                cipher.setAutoPadding(false);
                const encrypted = Buffer.concat([cipher.update(payload), cipher.final()]);
                
                // Add signature AFTER encryption
                payload = Buffer.concat([encrypted, sign]);
            } else {
                payload = Buffer.concat([counter, payload]);
            }
        }
    ...
    // Zwei-Phasen Query: erst SET (mit echten Daten), dann POLL
    buildV3Query(device, phase = 'poll') {
        if (phase === 'set') {
            // SET-Kommando (echte Daten aus MessageSubProtocolSet)
            const set_data = Buffer.from([
                0xAA, 0x37, 0xAC, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03,
                0x02, 0xAA, 0x2D, 0x00, 0xFF, 0xFF, 0x20, 0x03, 0x80, 0x00,
                0x00, 0x00, 0x4E, 0x66, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x01, 0x01, 0x00, 0x01, 0x60, 0x00, 0x4E, 0x32, 0x66,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0xEB,
                0x66, 0x18
            ]);
            this.adapter.log.info('buildV3Query (SET): body=' + set_data.toString('hex'));
            return this.encode8370(set_data, 0x06);
        }
        
        // POLL-Query (Status abfragen)
        const query_data = Buffer.from([
        ...
        this.adapter.log.info('buildV3Query (POLL): body=' + query_data.toString('hex'));
        
        return this.encode8370(query_data, 0x06);
    }
