...
class MideaAdapter {
                    ...
                    socket.once('data', (d) => {
                        ...
                        if (d.length >= 64) {
                            ...
                            setTimeout(() => {
                                ...
                                const query = this.buildV3Query(device, this.queryPhase);
                                this.adapter.log.info('=== Sending query (' + this.queryPhase + ') after delay');
                                socket.write(query);
                                
                                // Toggle phase for next call
                                this.queryPhase = this.queryPhase === 'set' ? 'poll' : 'set';
                                
                                // Reset timeout AFTER sending query (like Python)
                                socket.setTimeout(30000);
                                
                                socket.once('data', (data) => {
                                    this.adapter.log.info('=== Got response: ' + data.toString('hex').substring(0, 40));
                                    
                                    // If SET phase, wait and send POLL
                                    if (this.queryPhase === 'set') {
                                        // After SET response, immediately send POLL
                                        setTimeout(() => {
    ...
    // Zwei-Phasen Query: erst SET (dummy), dann POLL
    buildV3Query(device, phase = 'poll') {
        if (phase === 'set') {
            // SET-Kommando (dummy) - initiiert die Verbindung
