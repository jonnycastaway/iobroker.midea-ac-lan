...
class MideaAdapter {
    ...
    decode8370(data) {
        ...
        try {
            ...
            if (msgType === 0x03 || msgType === 0x06) {
                ...
                // ACK response - try to decrypt
                this.adapter.log.info('decode8370: msgType=0x05');
                ...
                // Keep-alive/pong response - ignore
                this.adapter.log.info('decode8370: msgType=0x0F (keep-alive), ignoring');
                return [];
            } else {
    ...
    buildV3Query(device) {
        // Use the same format as Python's MessageQueryAppliance from msmart-ng
        // Body format: aa 1e ac 00 00 00 00 00 00 00 00 ... 00 checksum
        // Or body: 00 00 00 00 00 (5 bytes of zeros)
        
        // Try simple 5-byte query like some devices expect
        const body = Buffer.from([0x41, 0x01, 0x00, 0x00, 0x00]);  // 5 bytes
