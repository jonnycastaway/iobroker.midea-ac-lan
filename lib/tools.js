...
if (padLen > 0 && padLen <= 16 && padLen <= plain.length) {
    ...
    buildV3Query(device) {
        ...
        const devId = BigInt(device.id);
        payload.writeBigUInt64LE(devId, 0);
        payload[8] = 0x03;
        
        // Die ersten 7 Bytes im Query-Body sind das Gerät
        payload[9] = 0x10;  // SubType
        
        this.adapter.log.info('buildV3Query: device.id=' + device.id + ', payload=' + payload.toString('hex'));
        
        // Payload ist 16 bytes, aber die echte Query braucht padding + signature
        // Lass uns simpler anfangen
        return this.encode8370(Buffer.from('030a000000000000', 'hex'), 0x06);
    }
