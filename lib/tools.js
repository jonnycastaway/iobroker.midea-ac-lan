...
class MideaAdapter {
    ...
    decode8370(data) {
        ...
        try {
            ...
            this.adapter.log.info('decode8370: size=' + size + ', total=' + totalSize + ', data.length=' + data.length);
            ...
            this.adapter.log.info('decode8370: msgType=' + msgType.toString(16) + ', payload.len=' + payload.length);
            
            if (msgType === 0x03 || msgType === 0x06) {
                // Remove padding and MAC if present
                ...
                // ACK message - check if there's embedded encrypted data
                // Sometimes AC sends data in msgType 0x05 responses
                this.adapter.log.info('decode8370: ACK response, checking for data...');
                if (payload.length > 16) {
                    // Try to decrypt as it might contain encrypted data
                    try {
                        const decipher = crypto.createDecipheriv('aes-128-cbc', this.tcpKey, Buffer.alloc(16, 0));
                        const decrypted = Buffer.concat([decipher.update(payload.slice(0, -16)), decipher.final()]);
                        this.adapter.log.info('decode8370: decrypted ACK data: ' + decrypted.toString('hex'));
                        return [decrypted];
                    } catch (e) {
                        this.adapter.log.info('decode8370: could not decrypt ACK data');
                    }
                }
                return [];
            } else {
    ...
    buildV3Query(device) {
        // Midea query format - similar to msmart-ng
        // For status query: aa 14 ac 00 00 00 00 00 [body...] checksum
        
        // Use different query format based on what the device expects
        // Format: [flag, length, device_type, ...]
        
        // Query body for appliance status (0x41 = get status)
        const body = Buffer.from([
            0x41,  // Command: query status
            0x81,  // Some flags
        ...
        // Try with msgType 0x03 instead of 0x06
