...
class MideaAdapter {
    ...
    buildV3Query(device) {
        // Use the same format as Python's PacketBuilder for the inner packet
        // Then wrap it in encode8370 with msgType 0x06
        
        // Inner packet: 0x5A 0x5A header + device id + command
        ...
        const idStr = device.id.toString();
        const idHash = Math.abs(idStr.split('').reduce((a, c) => ((a << 5) - a) + c.charCodeAt(0), 0));
        const deviceIdNum = idHash % 0xFFFFFFFF;
        
        // Write device ID as 8 bytes little-endian
        const devIdBuf = Buffer.alloc(8);
        devIdBuf.writeUInt32LE(deviceIdNum & 0xFFFFFFFF, 0);
        devIdBuf.writeUInt32LE((deviceIdNum >> 32) & 0xFFFFFFFF, 4);
        ...
        // Set device ID at position 20 (similar to Python)
        // Just use simple approach with the numeric ID
        const idBytes = Buffer.alloc(8);
        for (let i = 0; i < 8; i++) {
            idBytes[i] = (deviceIdNum >> (i * 8)) & 0xFF;
        }
        idBytes.copy(inner, 2);
        
        // Add the actual query command (0x03 0x10)
        inner[8] = 0x03;
        inner[9] = 0x10;
        
        this.adapter.log.info('buildV3Query: device.id=' + device.id + ', num=' + deviceIdNum + ', inner=' + inner.toString('hex'));
        
        return this.encode8370(inner, 0x06);
    }
