...
class MideaAdapter {
                ...
                try {
                    ...
                    socket.once('data', (d) => {
                        ...
                        if (d.length >= 64) {
                            ...
                            // Step 1: Send appliance query first (like HA does)
                            const applianceQuery = this.buildV3ApplianceQuery();
                            this.adapter.log.info('=== V3: ApplianceQuery: ' + applianceQuery.toString('hex'));
                            socket.write(applianceQuery);
                            
                            let applianceTimeout = setTimeout(() => {
                                this.adapter.log.warn('=== V3: Appliance query timeout');
                            }, 5000);
                            
                            socket.once('data', (applianceResp) => {
                                clearTimeout(applianceTimeout);
                                this.adapter.log.info('=== V3: Appliance Response: ' + applianceResp.toString('hex') + ' (' + applianceResp.length + ' bytes)');
                                
                                // Step 2: Now send status query
    ...
    buildV3Query(device) {
        ...
        // aa 1d ac 00 00 00 00 00 a0 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 97
    ...
    buildV3ApplianceQuery() {
        ...
        // This is sent with msgType 0x03 but gets a different response
        const body = Buffer.from([
