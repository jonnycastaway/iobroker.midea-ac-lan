...
initTcpKey(response, key) {
        try {
            ...
            this.adapter.log.info('=== Trying aes-256-cbc');
            try {
                ...
                this.adapter.log.info('=== SUCCESS aes-256-cbc');
            } catch(e) {
                this.adapter.log.info('=== Failed aes-256-cbc: ' + e.message);
                
                this.adapter.log.info('=== Trying aes-256');
                try {
                    const decipher = crypto2.createDecipheriv('aes-256', key, iv);
                    decipher.setAutoPadding(true);
                    plain = Buffer.concat([decipher.update(payload), decipher.final()]);
                    this.adapter.log.info('=== SUCCESS aes-256');
                } catch(e2) {
                    this.adapter.log.info('=== Failed aes-256: ' + e2.message);
                    
                    this.adapter.log.info('=== Trying aes-128-cbc with key[:16]');
                    try {
                        const decipher = crypto2.createDecipheriv('aes-128-cbc', key.slice(0, 16), iv);
                        decipher.setAutoPadding(true);
                        plain = Buffer.concat([decipher.update(payload), decipher.final()]);
                        this.adapter.log.info('=== SUCCESS aes-128-cbc');
                    } catch(e3) {
                        this.adapter.log.info('=== Failed aes-128-cbc: ' + e3.message);
                        throw new Error('All AES approaches failed');
                    }
                }
            }
    ...
    buildV3Query(device) {
        ...
        payload.writeUInt32LE(parseInt(device.id), 0);
        payload[8] = 0x03;
        payload[9] = 0x10;
        
        this.adapter.log.info('buildV3Query: device.id=' + device.id + ', payload=' + payload.toString('hex'));
        
        return this.encode8370(payload, 0x06);
    }
