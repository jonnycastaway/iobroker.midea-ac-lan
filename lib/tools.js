...
class MideaAdapter {
    ...
    buildV3Query(device) {
        ...
        // Device ID is too large for UInt32, use smaller representation
        // Just use a simple counter or hash
        const deviceIdNum = parseInt(device.id.toString().slice(-8), 10) % 0xFFFFFFFF;
        payload.writeUInt32LE(deviceIdNum, 0);
        
        payload[8] = 0x03;
        payload[9] = 0x10;
        
        this.adapter.log.info('buildV3Query: device.id=' + device.id + ', num=' + deviceIdNum + ', payload=' + payload.toString('hex'));
