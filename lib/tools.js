...
class MideaProtocol {
    ...
    calculateTcpKey(responseData) {
        ...
        for(let i = 0; i < 32; i++) {
            if (hash[i] !== signature[i]) return null;
        }
    ...
    encode8370(data, msgType) {
        ...
        // Check if encrypted type - but NOT for handshake
        if ((msgType === MSGTYPE_ENCRYPTED_REQUEST || msgType === MSGTYPE_ENCRYPTED_RESPONSE) && msgType !== MSGTYPE_HANDSHAKE_REQUEST) {
            if ((size + 2) % 16 !== 0) {
        ...
        // Build header: 0x83 0x70 size(2) 0x20 flags
        const header = Buffer.alloc(6);
        header[0] = 0x83;
        header[1] = 0x70;
        header.writeUInt16BE(size, 2);  // size
        header[3] = 0x20;
        header[4] = msgType;
        ...
        if ((msgType === MSGTYPE_ENCRYPTED_REQUEST || msgType === MSGTYPE_ENCRYPTED_RESPONSE) && msgType !== MSGTYPE_HANDSHAKE_REQUEST) {
            ...
            const signData = Buffer.concat([header.slice(0, 4), finalData]);
            const signature = crypto.createHash('sha256').update(signData).digest();
            
            return Buffer.concat([header.slice(0, 4), encrypted, signature]);
        }
        
        // Simple (handshake) packet: just header + payload
...
class MideaAC {
    ...
    sendHandshake() {
        this.log.info(`[${this.deviceId}] Token bytes: ${this.protocol.token.length}`);
        const req = this.protocol.encode8370(this.protocol.token, MSGTYPE_HANDSHAKE_REQUEST);
        this.log.info(`[${this.deviceId}] Handshake: ${req.toString('hex')}`);
        this.socket.write(req);
    }
    
    processMessage(packet) {
        this.log.info(`[${this.deviceId}] Message: ${packet.length} bytes`);
        if (!packet || packet.length < 8) return;
        
        const size = packet.readUInt16BE(2) + 8;
        const typeField = packet[3];
        ...
        if (typeField === 0x20 && packet.length >= 72 && !this.connected) {
            ...
            // IMPORTANT: Wait 3 seconds before sending queries
