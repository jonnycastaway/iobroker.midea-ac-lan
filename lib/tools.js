...
class MideaAC {
    ...
    sendHandshake() {
        ...
        this.log.info(`[${this.deviceId}] msgType=${MSGTYPE_HANDSHAKE_REQUEST}, payloadLen=${payload.length}`);
        
        // Test: manually create frame
        const frame = Buffer.alloc(8 + payload.length);
        frame[0] = 0x83;
        frame[1] = 0x70;
        ...
        this.log.info(`[${this.deviceId}] Manual frame bytes 0-7: ${frame[0].toString(16)} ${frame[1].toString(16)} ${frame[2].toString(16)} ${frame[3].toString(16)} ${frame[4].toString(16)} ${frame[5].toString(16)} ${frame[6].toString(16)} ${frame[7].toString(16)}`);
        this.log.info(`[${this.deviceId}] Full: ${frame.toString('hex')}`);
