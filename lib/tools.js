...
class MideaAdapter {
    ...
    buildV3Query(device) {
        // Try a simpler query format - similar to Python's MessageQueryAppliance
        // The format is: 0x5A 0x5A + device_type (0xAC) + body
        ...
        // Message type: query_appliance = 0xA0 (or 0x03 for query)
        // Let's try 0xA0 first
        inner[3] = 0xA0;
        
        // Body type
        inner[4] = 0x00;
        
        // Body: 19 bytes of zeros (like Python's MessageQueryAppliance)
        // Bytes 5-15 are already 0 from alloc
        // Just fill remaining
        ...
        // The inner packet also includes device ID in some format
        // For now, let's use a simpler approach
        
        this.adapter.log.info('buildV3Query: device.id=' + device.id + ', num=' + deviceIdNum + ', inner=' + inner.toString('hex'));
        
        // For now, try msgType 0x03 instead of 0x06 - maybe that's what causes the error
        return this.encode8370(inner, 0x03);
    }
