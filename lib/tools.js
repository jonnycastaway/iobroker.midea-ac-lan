...
class NetHomeCloud {
    ...
    async request(path, data, timeoutMs = 10000, includeSession = true) {
        return new Promise((resolve, reject) => {
            ...
            // For sign: same as Python's unquote_plus(urlencode(...))
            // Decode %XX to chars AND convert + to space
            const queryParts = sortedKeys.map(k => [k, body[k]]);
            const queryEncoded = new URLSearchParams(queryParts).toString();
            const queryForSign = queryEncoded.replace(/%([0-9A-Fa-f]{2})/g, (m, h) => String.fromCharCode(parseInt(h, 16))).replace(/\+/g, ' ');
            ...
            // For post data: use URL encoded format
            const postData = new URLSearchParams(sortedKeys.map(k => [k, String(body[k])])).toString();
