...
class MideaAdapter {
    ...
    decode8370(data) {
        ...
        try {
            ...
            if (msgType === 0x03 || msgType === 0x06) {
                ...
                // ACK response - could contain data or be just an ACK
                this.adapter.log.info('decode8370: msgType=0x05, payload=' + payload.toString('hex'));
                
                // If payload is large enough, try to decrypt it
                if (payload.length >= 16) {
                    try {
                        // Try decrypting without removing anything first
                        const decipher = crypto.createDecipheriv('aes-128-cbc', this.tcpKey, Buffer.alloc(16, 0));
                        const decrypted = Buffer.concat([decipher.update(payload), decipher.final()]);
                        this.adapter.log.info('decode8370: decrypted 0x05: ' + decrypted.toString('hex'));
                        return [decrypted];
                    } catch (e) {
                        this.adapter.log.info('decode8370: 0x05 decrypt failed: ' + e.message);
                    }
                }
                
                // Return raw payload if small (might be status data)
                if (payload.length > 2) {
                    return [payload.slice(2)];  // Skip first 2 bytes
                }
                return [];
            } else {
    ...
    buildV3Query(device) {
        // Use msgType 0x06 which worked before (got response)
        const body = Buffer.from([
            0x41, 0x81, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x00,
        ...
        // Use msgType 0x06 (we know it responds)
