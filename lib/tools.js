...
class MideaAdapter {
    ...
    buildV3Query(device) {
        // Full AC query: body_type (0x41) + body (19 bytes) + message_id + CRC = 22 bytes
        const payload = Buffer.alloc(22);
        payload[0] = 0x41;  // body_type
        payload[1] = 0x81;  // query header
        payload[3] = 0xFF;  // query trailer
        payload[20] = 1;  // message_id
        
        this.adapter.log.info('buildV3Query: body=' + payload.toString('hex'));
