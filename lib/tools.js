...
class MideaAC {
    ...
    handleData(data) {
        ...
        if (!this.connected) {
            this.log.info(`[${this.deviceId}] Not connected yet, checking handshake response...`);
            
            if (data.length >= 8) {
                const msgType = data[4];
                this.log.info(`[${this.deviceId}] Message type: 0x${msgType.toString(16)}`);
            }
            
            if (data.length >= 72) {
                const responseData = data.slice(8, 72);
                this.protocol.tcpKey(responseData);
                this.connected = true;
    ...
    buildQueryAppliance() {
        ...
        header.writeUInt32BE(parseInt(this.deviceId), 3);
        header[7] = 0x00;
        header[8] = 0x00;
        header[9] = 0xA0;
