...
class NetHomeCloud {
    ...
    async getToken(deviceId) {
        ...
        if (data.tokenlist && data.tokenlist.length > 0) {
            // Find matching token
            const token = data.tokenlist.find(t => t.udpId === deviceId || t.udpId === deviceId.substring(deviceId.length - 32));
            if (token) {
                return {
                    token: token.token,
                    key: token.key,
            ...
            // Return first token if no match found
            return {
    ...
    async fetchTokensForDevices() {
        ...
        try {
            ...
            for (const cloudDev of cloudDevices) {
                this.adapter.log.info('Cloud device: id=' + cloudDev.id + ' name=' + cloudDev.name + ' ip=' + cloudDev.wifiIp + ' type=' + cloudDev.type);
                
                // Match by IP or by derived ID
                const ipMatch = Array.from(this.devices.values()).find(d => 
                    d.ipAddress === cloudDev.wifiIp || d.id === parseInt(cloudDev.id, 10)
                );
                
                if (ipMatch) {
                    this.adapter.log.info('Matched device: ' + ipMatch.ipAddress + ' with cloud id: ' + cloudDev.id);
                    const tokenData = await this.cloud.getToken(cloudDev.id);
                    if (tokenData) {
