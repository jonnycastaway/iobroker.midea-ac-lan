...
this.adapter.log.info('=== DEBUG sign=' + sign.toString('hex').substring(0, 32));
            ...
            const approaches = [
                { algo: 'aes-256-cbc', key: key },
                { algo: 'aes-256', key: key },
                { algo: 'aes-128-cbc', key: key.slice(0, 16) },
            ];
            
            let success = false;
            for (const { algo, k } of approaches) {
                try {
                    this.adapter.log.info('=== Trying: ' + algo);
                    const decipher = crypto2.createDecipheriv(algo, k, iv);
                    decipher.setAutoPadding(true);
                    plain = Buffer.concat([decipher.update(payload), decipher.final()]);
                    this.adapter.log.info('=== SUCCESS: ' + algo + ' -> ' + plain.toString('hex').substring(0, 32));
                    success = true;
                    break;
                } catch(e) {
                    this.adapter.log.info('=== Failed: ' + algo + ' -> ' + e.message);
                }
            }
            ...
            if (!success) {
                ...
                try {
                    const decipher = crypto2.createDecipher('aes-256-cbc', key);
                    decipher.setAutoPadding(true);
                    plain = Buffer.concat([decipher.update(payload), decipher.final()]);
                    this.adapter.log.info('=== LEGACY Worked');
                } catch(e) {
