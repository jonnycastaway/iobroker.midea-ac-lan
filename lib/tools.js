...
if (padLen > 0 && padLen <= 16 && padLen <= plain.length) {
    ...
    encode8370(data, msgType) {
        ...
        if (msgType === 0x00) {
            ...
            if ((msgType === 0x03 || msgType === 0x06) && this.tcpKey) {
                ...
                // Use AES-256 like Python (full tcpKey)
    ...
    buildV3Query(device) {
        // Full query structure: body_type + body + message_id + CRC
        // body_type = 0x41, body = 19 bytes, message_id = 1 byte, CRC = 1 byte = 22 total
        const buf = Buffer.alloc(22);
        buf[0] = 0x41;  // body_type
        buf[1] = 0x81;  // query header
        buf[3] = 0xFF;  // query trailer
        // Rest are zeros
        
        // message_id would be assigned here, but start with 1
        buf[20] = 1;  // message_id
        
        // CRC8 would be calculated - for now use placeholder
        // We'll skip CRC for testing
        
        this.adapter.log.info('buildV3Query: body=' + buf.toString('hex'));
        
        return this.encode8370(buf, 0x06);
    }
