...
class NetHomeCloud {
    ...
    async request(path, data, timeoutMs = 10000, includeSession = true) {
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < sortedKeys.length; i++) {
                ...
                query += key + '=' + value;
                if (i < sortedKeys.length - 1) query += '&';
            }

            const msg = path + query + NETHOME_CONFIG.APP_KEY;
            this.log('debug', 'Sign msg: ' + msg);
            const signValue = crypto.createHash('sha256').update(msg).digest('hex');
            this.log('debug', 'Sign result: ' + signValue);
            body.sign = signValue;
            ...
            this.log('debug', 'Post data: ' + postData.substring(0, 200));
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    try {
                        this.log('debug', 'NetHome response: ' + responseBody.substring(0, 500));
                        const json = JSON.parse(responseBody);
    ...
    async login(email, password) {
        ...
        this.log('debug', 'Password hash: ' + encryptedPassword);
