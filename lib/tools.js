...
class MideaAdapter {
    ...
    async queryDevice(device) {
        this.adapter.log.info('=== queryDevice: protocol=' + device.protocol + ', hasToken=' + !!device.token + ', hasKey=' + !!device.key);
        
        // Try msmart-style query first (non-encrypted V3 protocol)
        if (device.protocol === 3 && device.token && device.key) {
            this.adapter.log.info('=== queryDevice: Calling Msmart query');
            try {
