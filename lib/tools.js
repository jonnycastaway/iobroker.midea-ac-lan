...
this.adapter.log.info('=== DEBUG sign=' + sign.toString('hex').substring(0, 32));
            ...
            // Try single .decrypt() call like Python does
            let plain;
            
            try {
                const iv = Buffer.alloc(16, 0);
                const decipher = crypto.createDecipheriv('aes-256-cbc', key, iv);
                // Single decrypt call like Python (no update+final)
                plain = decipher.decrypt(payload);
                this.adapter.log.info('=== DEBUG AES decrypt single call worked');
            } catch(e) {
                this.adapter.log.info('=== AES decrypt fail: ' + e.message);
                throw e;
            }
